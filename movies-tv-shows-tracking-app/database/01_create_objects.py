...
def create_database_objects():
    ...
    # Create OTHER_LINKS table (if not exists) (Type can be trailer or watch link => TRAILER, WATCH_LINK, OTHER)
    cursor.execute("""
    ...
    FROM
        ...
        INNER JOIN
            ...
            FROM EPISODES
            WHERE 1=1
                AND CASE WHEN WATCHED_DATE IS NULL OR WATCHED_DATE = '1900-01-01' THEN 0 ELSE 1 END = 1
            GROUP BY RELATED_ID
            ) B
    ...
    LEFT JOIN
        ...
        SELECT RELATED_ID, 
            ...
            MAX(CASE WHEN WATCHED_DATE <> '1900-01-01' THEN EPISODE_NO ELSE 0 END) OVER (PARTITION BY RELATED_ID) AS LAST_WATCHED_EPISODE_NO
        FROM EPISODES
        ) B
